    ...
    for iline in best_obj:
        ...
        if 'potfile' in line[0]:
            ...
            if len(line[0].split()) == 1:
                if line[0].split()[0] != 'potfile':
                    iPotfileName = line[0].split()[0][7:]
                else:
                    #default name
                    iPotfileName='galaxies'
            else:
                iPotfileName = line[0].split()[1]
            #Assuming that we have a redshift already
            #and potfile is at the bottom of the pack
            z_lens = run_mode['mass']['z_lens']
            #there maube many potfiles
            if iPotfileName != PotfileName:
                iPotFile = potfile(iPotfileName, z_lens)
                PotfileName = iPotfileName
        ...
        if len(line) > 1:
            ...
            if line[1].strip() != 'end':
                ...
                if (mode == 'potfile'):
                    option=line[1].split()
                    if not option[0] in iPotFile.keys():
                        if verbose:
                            print option[0]+' not in '+mode
                        continue
                    keys = iPotFile[option[0]].dtype.names
                     
                    for iKey in xrange(len(keys)):
                       
                        if iKey < len(option):
                            
                            iPotFile[option[0]][keys[iKey]] = option[iKey]
                        else:
                            continue
                    
            else:
                if mode == 'potfile':
                    ret_potfile.append(cp(iPotFile))
                mode = 'end'
...
def add_galaxy_lenses( potentiel_list, potfiles, par_dir, \
                           nlens, nlens_done ):
    '''
    ...
    for pot_file in potfiles:
        ...
        print pot_file['filein']['filename']
        for i in xrange(len(galaxy_members['id'])):
            
            iPot = potentiels.get_profile( pot_file['type']['int'])
            ellipticity = (galaxy_members['a'][i]**2 - \
                               galaxy_members['b'][i]**2)/ \
